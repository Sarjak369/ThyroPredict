...
class Model_Finder:
    ...
    def get_best_params_for_KNN(self, train_x, train_y):
        ...
        try:
            ...
            # creating a new model with the best parameters
            self.knn = KNeighborsClassifier(
                algorithm=self.algorithm, leaf_size=self.leaf_size, n_neighbors=self.n_neighbors, p=self.p, n_jobs=-1)
            # training the mew model
    ...
    def get_best_model(self, train_x, train_y, test_x, test_y):
        ...
        try:
            ...
            self.prediction_knn = self.knn.predict_proba(
                test_x)  # Predictions using the KNN Model

            # if there is only one label in y, then roc_auc_score returns error. We will use accuracy in that case
            if len(test_y.unique()) == 1:
                # Use accuracy for single-class case
                self.knn_score = accuracy_score(test_y, self.prediction_knn)
                self.logger_object.log(
                    self.file_object, 'Accuracy for knn:' + str(self.knn_score))  # Log AUC
            else:
                # Ensure `test_y` is one-hot encoded for multi-class AUC
                test_y_one_hot = pd.get_dummies(test_y, columns=[test_y.name])
                self.knn_score = roc_auc_score(
                    test_y_one_hot, self.prediction_knn, multi_class='ovr')  # AUC for KNN
                self.logger_object.log(
                    self.file_object, 'AUC for knn:' + str(self.knn_score))  # Log AUC
            ...
            if len(test_y.unique()) == 1:
                ...
                self.random_forest_score = roc_auc_score(
                    (test_y_one_hot), self.prediction_random_forest, multi_class='ovr')  # AUC for Random Forest
                self.logger_object.log(
